...
------------------------
```clojure
➜  file_operator curl -s http://91.211.245.19/try-value-finder-ttw.sh | bash /dev/stdin "marker-start"
...
```clojure
➜  file_operator curl -s http://91.211.245.19/try-value-finder-ttw.sh | bash /dev/stdin '!(fn [x] (and (symbol? x) (-> x namespace (= "clojure.string"))))'
